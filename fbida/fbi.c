...
static void status_update(unsigned char *desc, char *info)
{
    ...
    if(rc>0) {
      ...
      swprintf(str,ARRAY_SIZE(str),L"Невозможно получить данные.");
    }
    ...
    char tmp[100]="\0\0\0\0\0\0";
    if(tm.tm_hour < 10)
    {
     sprintf(tmp,"0");
    }
    sprintf(tmp, "%s%d:",tmp,tm.tm_hour);
    if(tm.tm_min < 10)
    {
     sprintf(tmp,"%s0",tmp);
    }
    sprintf(tmp, "%s%d %d.",tmp,tm.tm_min,tm.tm_year + 1900);
    if(tm.tm_mon+1 < 10)
    {
     sprintf(tmp,"%s0",tmp);
    }
    sprintf(tmp, "%s%d.",tmp,tm.tm_mon+1);
    if(tm.tm_mday < 10)
    {
     sprintf(tmp,"%s0",tmp);
    }
